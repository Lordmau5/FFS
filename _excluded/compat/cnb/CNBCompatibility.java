...
public enum CNBCompatibility {
    ...
    public void onBlockBitModified(EventBlockBitPostModification event) {
        ...
        if ( !FancyFluidStorage.TANK_MANAGER.isPartOfTank(event.getWorld(), event.getPos()) ) {
            return;
        }

        AbstractTankValve valve = FancyFluidStorage.TANK_MANAGER.getValveForBlock(event.getWorld(), event.getPos());
