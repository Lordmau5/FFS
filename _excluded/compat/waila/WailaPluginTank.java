...
//            valve = (AbstractTankValve) te;
            ...
            valve = ((AbstractTankTile) te).getMainValve();
            if ( valve != null && valve.isValid() )
                list.add("Part of a tank");
        }

        if ( valve == null && FancyFluidStorage.TANK_MANAGER.isPartOfTank(iWailaDataAccessor.getWorld(), iWailaDataAccessor.getPosition()) ) {
            valve = FancyFluidStorage.TANK_MANAGER.getValveForBlock(iWailaDataAccessor.getWorld(), iWailaDataAccessor.getPosition());
            if ( valve != null && valve.isValid() ) {
                list.add("Part of a tank");
            }
        }
    ...
    @Optional.Method(modid = "waila")
    @Override
    public NBTTagCompound getNBTData(ServerPlayerEntity serverPlayerEntity, TileEntity tileEntity, NBTTagCompound nbtTagCompound, World world, BlockPos pos) {
        // Unused, implemented because of the interface
