	...
	public void onRightClick(PlayerInteractEvent.RightClickBlock event) {
		...
		if(event.getEntityPlayer().isSneaking()) {
			...
			if(mainHand != null) {
				...
				mainHand.onItemUse(event.getEntityPlayer(), event.getWorld(), event.getPos(), EnumHand.MAIN_HAND, event.getFace(), (float) event.getHitVec().xCoord, (float) event.getHitVec().yCoord, (float) event.getHitVec().zCoord);
				event.getEntityPlayer().swingArm(EnumHand.MAIN_HAND);
			}
			return;
		}
